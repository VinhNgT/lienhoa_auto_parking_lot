...
class LedsPcf8574:
    ...
    def __init__(self, i2c, address=0x20, led_count=8, reverse_layout=False):
    ...
    def set_leds_byte(self, byte: int):
        if byte < 1 or byte > self.max_byte:
            raise ValueError(
                f"byte must be between 0x01 and {hex(self.max_byte)}"
            )

        # Flip bits because LEDs are active low.
        byte = ~byte & 0xFF
        self.pcf.write_gpio(
            byte if not self.reverse_layout else self._reverse_byte(byte)
        )
...
with LedsPcf8574(I2C(7), reverse_layout=True, led_count=5) as led_ctl:
    ...
    # led_ctl.set_leds_byte(0b00011011)
    # time.sleep(1)
